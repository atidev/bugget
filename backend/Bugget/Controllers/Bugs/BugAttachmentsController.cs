...
[Route("/v2/reports/{reportId}/bugs/{bugId}/attachments")]
public sealed class BugAttachmentsController(AttachmentService attachmentService) : ApiController
{
    ...
    public async Task<IActionResult> GetAttachmentContentAsync([FromRoute] int reportId, [FromRoute] int bugId, [FromRoute] int id)
    {
        var user = User.GetIdentity();
        var attachResult = await attachmentService.GetBugAttachmentContentAsync(user, reportId, bugId, id);
    ...
    [ProducesResponseType(typeof(FileStreamResult), 200)]
    public async Task<IActionResult> GetAttachmentPreviewContentAsync([FromRoute] int reportId, [FromRoute] int bugId, [FromRoute] int id)
    {
        var user = User.GetIdentity();
        var attachResult = await attachmentService.GetBugAttachmentPreviewContentAsync(user, reportId, bugId, id);
        if (attachResult.HasError)
            return attachResult.AsActionResult();
    ...
    public Task DeleteAttachmentAsync([FromRoute] int reportId, [FromRoute] int bugId, [FromRoute] int id)
    {
        var user = User.GetIdentity();
        return attachmentService.DeleteBugAttachmentAsync(user, reportId, bugId, id);
    }
}