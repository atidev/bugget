...
DECLARE
    ...
    summary jsonb;
    participants jsonb;
    is_participants_changed boolean := FALSE;
BEGIN
    -- Проверка доступа
    SELECT
        r.id INTO report_id
    ...
    IF NOT FOUND THEN
        ...
        IF NOT EXISTS (
            ...
            FROM
                public.report_participants as rp
            WHERE
                rp.report_id = _report_id
                AND user_id = _user_id) THEN
        INSERT INTO public.report_participants(report_id, user_id)
            VALUES (_report_id, _user_id);
        is_participants_changed := TRUE;
    END IF;
    -- Сборка результата
    summary := public.get_report_summary(_report_id, _organization_id);
    SELECT
        COALESCE(jsonb_agg(user_id), '[]'::jsonb) INTO participants
    FROM
        public.report_participants as rp
...
BEGIN
    ...
    SELECT jsonb_build_object(
                   ...
                   'past_responsible_user_id', r.past_responsible_user_id,
