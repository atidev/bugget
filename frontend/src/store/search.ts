...
export const $sortField = createStore<string>('created').on(updateSortField, (_, field) => field);
export const $sortDirection = createStore<'asc' | 'desc'>('desc').on(updateSortDirection, (_, direction) => direction);
export const $statuses = createStore<number[] | null>(null).on(updateStatuses, (_, s) => s);
...
sample({
    ...
    clock: [$query.updates, $sortField.updates, $sortDirection.updates, $statuses.updates],
    fn: ({ query, sortField, sortDirection, reportStatuses }) => ({
        query,
        sort: `${sortField}_${sortDirection}`,
        reportStatuses: reportStatuses ?? undefined,
